...
import { clearedCookieOptions, httpOnlyCookieOptions, readableCookieOptions } from "./auth.cookies";
import { SigninInput, SignupInput, UpdatePasswordInput } from "./auth.validation";

export async function signup(req: Request, res: Response) {
  const { token, user } = await authService.signup(req.body as SignupInput);

  res.status(STATUS_CODE.CREATED).cookie("Token", token, readableCookieOptions()).json({
...
export async function signin(req: Request, res: Response) {
  ...
  res.status(STATUS_CODE.OK).cookie("Token", token, readableCookieOptions()).json({
...
export async function updatePassword(req: Request, res: Response) {
  ...
  res.status(STATUS_CODE.CREATED).json({ message: STATUS_MESSAGE.PASSWORD_UPDATE_SUCCESS });
}
...
export function googleCallback(req: Request, res: Response): void {
  if (!req.isAuthenticated() || !req.user) {
    res.status(STATUS_CODE.UNAUTHORIZED).json({ error: true, message: STATUS_MESSAGE.UNAUTHORIZED });
    return;
  }
...
export function loginSuccess(req: Request, res: Response): void {
  ...
  const token = authService.signToken(user.email);

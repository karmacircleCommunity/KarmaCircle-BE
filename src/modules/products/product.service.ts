...
export async function addProduct(data: AddProductInput): Promise<IProduct> {
  ...
  if (existingSlug) {
    throw new AppError(STATUS_CODE.CONFLICT, STATUS_MESSAGE.PRODUCT_SLUG_ALREADY_EXISTS);
  }
...
export async function addToCart(email: string, productId: string): Promise<boolean> {
  const response = await User.updateOne({ email }, { $push: { cart: { id: productId } } });
